...
def training_step(model, X_batch, y_batch, reg):
    ...
    ###########################################################################
    scores, cache = model.forward(X_batch) #forward pass
    data_loss, grad_scores = softmax_loss(scores, y_batch) #compute the softmax cross-entropy loss and grad
    reg_loss = 0.0
    for param_name, param_value in model.parameters().items():
        if 'W' in param_name:  #check if the parameter is a weight matrix
            reg_loss += l2_regularization(param_value, reg)[0]
    loss = data_loss + reg_loss
    grads = model.backward(grad_scores, cache) #perform backward pass to compute gradients
    for param_name, param_value in model.parameters().items():
        if 'W' in param_name:
            grads[param_name] += reg * param_value
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return loss, grads
